...
% 6
%esta mal xq repite logica en cocina en y eso se puede poner una sola funcion xq es un chef x restaurante
...
/*como lo hizo mate, pensando que en una restaurante habian varios chefs

tieneMayorRepertorio(RestauranteMayor, RestauranteMenor) :-
    cocinaEn(RestauranteMayor, ChefMayor),
    cocinaEn(RestauranteMenor, _),
    RestauranteMayor \= RestauranteMenor,
    cantidadDePlatos(ChefMayor, CantidadMayor),
    forall(
        cocinaEn(RestauranteMenor, ChefMenor),
        (cantidadDePlatos(ChefMenor, CantidadMenor), CantidadMayor > CantidadMenor)
    ).
